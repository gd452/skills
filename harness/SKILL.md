...
> **출처**: [revfactory/harness](https://github.com/revfactory/harness) (별도 repo) · 도입 시점 미기록
> 로컬 변경: 진행 중 — 커스텀 작업 중. 최근 추가: "다음 단계 — 외부 모델 교차검증 (권장)" 섹션 (gd-multi-ai-review 권장)
...
하네스 설계 완료 후, `gd-multi-ai-review` 스킬이 설치돼 있다면 **에이전트 팀 구성·역할 분리·도메인 스킬 선택**이 합리적인지 Gemini + Codex 로 cross-check 권장. 잘못된 하네스는 모든 후속 작업의 효율을 깎아먹기 때문에 cost-effective 한 검증 시점.

```
사용자: "/harness ... 한 후 multi-ai-review 도 돌려줘"
→ harness 가 .claude/agents/, .claude/skills/, CLAUDE.md 포인터 등 산출 후
→ 그 산출물을 brief 로 multi-ai-review 자동 호출
```
